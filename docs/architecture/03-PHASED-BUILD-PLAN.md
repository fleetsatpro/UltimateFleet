# Phased Build Plan — Sonalit Guard Operations Platform

**Companion to:** `01-ARCHITECTURE.md`, `02-REPOSITORY-STRUCTURE.md` · **Status:** Draft for review

Every acceptance criterion below is a **runnable command with an observable pass/fail outcome**.
None is "it works."
...
flowchart LR
  ...
  TC["Track C: Compliance<br/>DPIA + ODPC — 60d lead"]
...
**Track C starts on day 1 of P1, in parallel.** It is not an engineering phase and consumes no
engineering capacity beyond supporting counsel, but it has a **verified ≥60-day statutory lead
time** (`01-ARCHITECTURE.md` §10.1) and it hard-gates P9. Starting it when P9 starts stalls P9 for
two months. This is the single highest-leverage scheduling decision in the plan.

---

## Phase 1 — Foundation & Tenant-Isolated Data Layer

**Scope: L** · **Depends on:** nothing (blocked only by Open Item 0, repository placement)

One sentence: stand up the monorepo, the strict toolchain, and the complete PostgreSQL schema with
row-level tenant isolation proven by test.

### Built

1. pnpm workspace, `tsconfig.base.json` with all three mandated flags, ESLint/Prettier shared config
   including the `Promise.all` ban and migration-lint, `check:boundaries`, CI pipeline.
2. `@sonalit/contracts` — every refined type and zod schema from `01-ARCHITECTURE.md` §6.
3. `@sonalit/config-env` — zod env validation, fail-hard at boot.
4. `@sonalit/observability` — pino JSON logger, `AsyncLocalStorage` correlation context.
5. `@sonalit/db`:
   - node-pg-migrate migrations (**up + down**) for all 13 core tables plus `alarm_closures`
   - three roles: `sonalit_owner`, `sonalit_app`, `sonalit_readonly`; GRANTs
   - `ENABLE` + **`FORCE` ROW LEVEL SECURITY** and `USING` + `WITH CHECK` policies on every
     tenant-scoped table
   - all indexes from `01-ARCHITECTURE.md` §11.2 including the partial open-alarms index
   - `withClientId()` / `withGlobalConfig()`; raw pool unexported
   - deterministic seeds: **2 clients**, 3 sites, 4 guards, 2 alarm sources, mapping rows
6. `@sonalit/test-support` — Testcontainers harness + cross-tenant assertion helpers.

### Acceptance criteria

`pnpm -r typecheck && pnpm -r lint && pnpm -r test:integration` passes in CI, including these
tests, each of which fails loudly if the corresponding control is absent:

| # | Test | Asserts |
|---|---|---|
| A1 | As `sonalit_app` with GUC = client A, `SELECT * FROM alarm_events` returns only A's seeded rows; count matches A's seed exactly | Basic RLS read isolation |
| A2 | As `sonalit_app` with GUC = client A, `INSERT` with `client_id` = B **raises** `new row violates row-level security policy` | `WITH CHECK` present — read-only isolation is not enough |
| A3 | As `sonalit_app` with **no GUC set**, every tenant table returns **0 rows** | Fails closed; a missing GUC never means "all tenants" |
| A4 | As `sonalit_owner` (the table owner) with GUC = client A, `SELECT` returns only A's rows | **`FORCE ROW LEVEL SECURITY` is actually applied** — this test fails if `FORCE` was omitted, which is the single most commonly missed RLS step |
| A5 | 100 interleaved `withClientId()` calls alternating A/B across a pool of 5 connections: zero cross-tenant rows, and `current_setting('app.current_client_id', true)` is NULL on a freshly checked-out connection | The GUC is transaction-local and does not leak to the next borrower of a pooled connection (`01-ARCHITECTURE.md` §7.2) |
| A6 | Same alarm event inserted twice → 1 row; second insert reports 0 rows affected | `(vendor, vendor_event_id)` dedupe works and is detectable by the caller |
| A7 | `pnpm run migrate:down:up` — all migrations down, then up, then schema matches a committed snapshot | Every `down()` genuinely reverses |
| A8 | Two active enrollments for one guard → unique violation; revoking the first then inserting → succeeds | Partial unique index `(guard_id) WHERE revoked_at IS NULL` enforces single-active-enrollment at DB level |
| A9 | Erase embedding (`embedding_vector = NULL`) → all of that guard's attendance/patrol rows still present and unchanged | Right-to-erasure is independent of the permanent audit trail (§11.4) |
| A10 | Service boots with a required env var missing → non-zero exit, error naming the variable; never a silent default | Fail-hard env contract |
| A11 | A fixture containing `Promise.all` fails `pnpm lint`; a fixture adding a `bytea` column outside the allowlist fails migration-lint | The enforcement rules are live, not aspirational |

A4 and A5 are the two tests worth reviewing most carefully — they are the tests that catch the RLS
mistakes that otherwise reach production looking like working code.
...
- `@sonalit/queue` with `SignedJobEnvelope` sign/verify inside the factory.
- Correlation IDs via `AsyncLocalStorage`; every log line carries
  `{ service, correlationId, clientId?, siteId?, vendor? }`.
- Ingestion metrics: success/failure per vendor.
- Fake adapters in `test-support` for all three ingestion modes.

### Acceptance criteria
1. Feeding 1,000 fake events (200 of them exact duplicates) through the pipeline yields **800 rows**
   and a duplicate-suppressed metric of exactly 200.
2. Fan-out fires exactly **800** times, not 1,000 — proving fan-out is gated on insertion, not on
   receipt.
...
7. A single request's correlation ID appears in every log line from HTTP entry through persistence
   through fan-out (asserted by parsing captured JSON logs).
...
- `@sonalit/resilience` — `createVendorPolicy()` with the **corrected** Cockatiel API (D1), breaker
...
1. Every adapter satisfies its interface under `pnpm -r typecheck`; **zero `any`** in the package
   (asserted by lint, no disables present).
...
3. Against a local fake SOAP/HTTP server: 5 consecutive failures open the breaker; the breaker gauge
   reads `open`; a 6th call fails fast **without** a network attempt (asserted by request count on
   the fake).
4. Retry timings across 3 attempts are **non-uniform across 20 runs** — proving jitter is active,
   not plain exponential.
5. A webhook with a valid signature is accepted; the **same body with one byte changed** is rejected
   `401`; a valid body sent through a `express.json()`-mounted route **fails the test** — the
...
### Acceptance criteria
1. Killing the fake Axxon endpoint and restoring it after 45 s: the worker reconnects, reconnect
   count increments, and **no events are lost** across the outage window.
2. Reconnect intervals over 10 induced failures are non-uniform, with the **first** retry already
   jittered.
3. The worker's environment contains no `DATABASE_URL` (asserted by its zod env schema rejecting
   one) and it holds zero DB connections under load.
...
**Scope: S** · **Depends on:** Phase 2 · **Constrained by:** Open Item 12

Persist expiring vendor media to R2 before the URLs die.

### Built
- `@sonalit/storage-r2`; BullMQ `media.fetch` worker; streaming multipart upload.
...
### Built
- Socket.io + Redis adapter on the engine; one room per `client:{id}`, authorized on connect.
...
### Acceptance criteria
1. Two browser clients on different tenants: an event for client A reaches A's socket and **never**
...
**Scope: M** · **Depends on:** Phase 1 · **Constrained by:** Open Item 11 (DNS)
...
### Acceptance criteria
1. An enrollment token is single-use: second redemption returns `410 Gone`. It expires (default
...
2. Setting `revoked_at` on an enrollment causes the **next refresh** to fail `401` and invalidates
   the token family.
3. Replaying an already-consumed refresh token invalidates the whole family and forces
   re-provisioning.
4. A `report_viewer` session receives `403` on every enrollment and revocation endpoint (asserted
   per route, table-driven, so a new route cannot be added unguarded).
5. Dashboard logout invalidates the session **server-side**: the same cookie returns `401`
   immediately after.
6. A service JWT signed with `SVC_SECRET_PREVIOUS` verifies during the rotation window; one signed
   with an unknown `kid` is rejected.
7. An expired service JWT (`exp` in the past) is rejected — replay is bounded.

---

## Phase 8 — Guard Mobile: Offline Event Log & Sync

**Scope: L** · **Depends on:** Phase 7 · **Constrained by:** Open Items 4, 10

Offline-first Android app with an immutable local event log. **No biometrics yet** — that is P9.
...
### Acceptance criteria
1. Airplane mode for a full simulated 12-hour shift: sign-in, 20 patrol scans, 3 alarm closures all
...
4. Sign-in 500 m outside a 100 m site radius: event persists with `geofence_violation = true` and
...
**Scope: L** · **Depends on:** Phase 8 · **HARD-GATED on Track C** · **Open Items 5, 6, 9**

> **This phase must not start** until ODPC registration and the DPIA are confirmed complete by
> counsel, **or** the client defers them **in writing** with accepted risk documented.
> `01-ARCHITECTURE.md` §10.1 establishes a verified ≥60-day statutory lead time.
...
1. Full enrollment against the selected SDK: `guard_enrollments` holds a non-null embedding, and a
   ...
   transmitted (asserted by inspecting the captured request bodies).
...
4. Below-threshold match: rejected; supervisor override succeeds and is recorded with the overriding
   user's ID.
5. Erasure: embedding NULL, erasure audit row written, and **every** historical attendance, patrol
   and closure record for that guard still present and byte-identical (re-asserts Phase 1 A9 against
   real data).
6. No `bytea`/`blob` column exists on any table other than `guard_enrollments.embedding_vector`
   (asserted against `information_schema`) — D5's boundary holds after the phase that was most
   likely to breach it.
...
### Built
- `apps/report-worker`: bounded Playwright pool, `newContext()` per render, recycle after N=50 and
  on an RSS ceiling.
...
### Acceptance criteria
1. 20 concurrent report jobs complete with **at most N+1 browser processes ever spawned**
   (asserted by process sampling) — proving pooling, not per-report launch.
...
4. Memory: 200 sequential renders with the pool's RSS returning to within 15% of baseline after
   recycling — no unbounded creep.
5. Report PDF byte-identical across two runs over identical frozen data (deterministic rendering).
6. Per-client report queries carry the tenant GUC: a report for A contains **zero** rows belonging
   to B (asserted by seeding a distinguishable marker into B's data).
...
2. Three recipients where one bounces: two `sent` rows, one `bounced` row, all three carrying the
   same `report_run_id` and `r2_archive_key`.
...
4. A poison-pill event lands in the DLQ, appears on the review surface, and does **not** stall the
   queue.
5. Load test: sustained 50 events/sec for 10 minutes with zero row loss and p95 ingest→dashboard
   under 2 s; documented breaking point.
...
|---|---|---|
| C1 | Kenyan counsel confirms ODPC registration status/obligation for Sonalit | Client-dependent |
...
| C3 | DPIA filed with the Data Commissioner | **≥60 days before processing begins** |
| C4 | Written sign-off, **or** written deferral with documented accepted risk | Gate for Phase 9 |

**Acceptance criterion:** a signed document in the project record either clearing biometric
processing or explicitly deferring it with accepted risk. Phase 9 does not start without it.
...
|---|---|---|
| P1 Foundation & Data Layer | L | Open Item 0 only |
...
| P5 Media Pipeline | S | Item 12 (R2 credentials) |
| P6 Ops Dashboard | M | No |
| P7 Auth & Enrollment | M | Item 11 (DNS) affects cookie policy only |
| P8 Guard Mobile | L | Items 4, 10 |
| P9 Biometric | L | **Hard-gated: Items 5, 6, 9 + Track C** |
| P10 Report Engine | M | No |
| P11 Delivery | S | Item 13 |
| P12 Hardening | M | Item 8 |

**Critical path:** P1 → P7 → P8 → P9, with P9 gated on Track C. Everything on the ingestion side
(P2–P6, P10–P12) proceeds in parallel with the mobile track, which is why P1's foundation work is
worth doing thoroughly rather than quickly — every other phase builds directly on its tenancy
guarantees.

**Two decisions needed before any code is written:** Open Item 0 (repository placement) and
authorization to start Track C. Everything else can be resolved as its phase approaches.
